...
impl Model {
    ...
    /// Remove solver options from the model.
    /// Options are now supplied via `SolveRequest` at the session boundary.
    #[deprecated(
        since = "0.1.0",
        note = "pass options via SolveRequest to BackendSession::solve"
    )]
    pub fn set_solver_options(&mut self, _opts: ()) {}
