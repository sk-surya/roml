...
pub(crate) fn compile(
    ...
    // recorded path is always the honest exact bridge — never a "native min/max"
    // label for a bridge formulation.
    select_path(
        ctx.capabilities,
        ctx.policy,
    ...
    finalizer.add_decision(FormulationDecision {
        ...
        reason: "no qualified native min/max; exact ROML bridge (design §8.1; F4)".to_string(),
    });
