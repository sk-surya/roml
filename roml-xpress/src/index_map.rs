//! Map from a typed ID to an Xpress column/row index.
//!
//! Xpress uses dense integer indices for columns and rows. When an element
//! is deleted at index `k`, Xpress closes the gap: everything at index > k
...
impl<Id: Hash + Eq + Copy> IndexMap<Id> {
    ...
    /// After Xpress deletes the element at `deleted`, decrement every stored
