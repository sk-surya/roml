...
pub(crate) fn rebuild_from_snapshot(
    ...
    obj_offsets: &mut HashMap<ObjId, f64>,
    ...
    *active_obj = None;
    ...
    //
    // CRITICAL: HiGHS supports only a single active objective. We must:
    // 1. Clear ALL HiGHS column costs to zero first (prevent stale costs
    //    from a previous rebuild blending with the new objective).
    // 2. Populate caches for ALL objectives (obj_costs, obj_senses,
    //    obj_offsets) for future delta operations and extraction.
    // 3. Project ONLY the active objective into HiGHS (costs, sense,
    //    constant offset).
    //
    // If no objective is active, HiGHS costs are already zeroed and
    // the sense defaults to minimize — the model simply has nothing
    // to optimize.

    // 7a: Clear ALL HiGHS column costs to zero.
    unsafe {
    ...
    // 7b: Populate per-objective caches and identify the active objective.
    for obj in &snapshot.objectives {
        obj_senses.insert(obj.id, obj.sense);
        obj_offsets.insert(obj.id, obj.constant);
    ...
    // 7c: Project ONLY the active objective into HiGHS.
    if let Some(active_id) = *active_obj {
        // Set costs from the active objective's coefficients.
        if let Some(costs) = obj_costs.get(&active_id) {
        ...
        // Set objective sense.
        if let Some(&sense) = obj_senses.get(&active_id) {
            unsafe {
                check_highs_status(
        ...
        // Set objective constant offset.
        if let Some(&offset) = obj_offsets.get(&active_id) {
            unsafe {
                check_highs_status(
    ...
    // If no active objective: HiGHS costs are already zeroed from step 7a
    // and the objective sense was cleared by Highs_clear in step 2.
    // The model will have no meaningful objective — any solve will minimize
    // zero, which is the expected degenerate behaviour.
        ...
        match op {
            ...
            ModelOp::RemoveObjective { obj } => {
                ...
                if *active_obj == Some(*obj) {
                    ...
                    unsafe {
                        ...
                        // Clear the objective offset since there is no active objective.
                        check_highs_status(
            ...
            ModelOp::SetActiveObjective { obj } => {
                ...
                if let Some(obj_id) = obj {
                    ...
                    // Restore objective offset from cache.
                    if let Some(&offset) = obj_offsets.get(obj_id) {
                        unsafe {
                            check_highs_status(
                                Highs_changeObjectiveOffset(raw, offset),
                                raw,
                    ...
                    // Clear the objective offset when no objective is active.
                    unsafe {
                        check_highs_status(
                            Highs_changeObjectiveOffset(raw, 0.0),
                            raw,
            ...
            ModelOp::SetObjectiveCell {
                ...
                // Store the constant offset in the per-objective cache.
                obj_offsets.insert(obj_id, *constant);

                // If this objective is currently active, immediately apply the
                // offset to HiGHS so that a subsequent solve() reflects the
                // updated constant in Highs_getObjectiveValue.
                if *active_obj == Some(obj_id) {
                    unsafe {
                        check_highs_status(
