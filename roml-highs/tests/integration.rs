//! Integration tests for HighsAdapter.
//!
...
// helper to initialise logging once for the integration test binary
// prints to stderr if setup fails so tests run in subdirectories can notice.
...
/// Drain the model changelog and apply to adapter.
fn sync(model: &mut Model, adapter: &mut HighsAdapter) {
...
fn parameter_coefficient_update() {
    ...
    assert_eq!(status, SolverStatus::Optimal);
    let sol = adapter.solution_values().unwrap();
    // x is still 1 (minimum), but now objective cost is 3*1 = 3
    assert!(approx_eq(sol[&x], 1.0), "expected x=1, got {}", sol[&x]);
}
...
fn binary_mip() {
    ...
    // Optimal: x=1, y=1, z=0 → 2+3=5<=5, obj=9
    assert!(approx_eq(obj_val, 9.0), "expected obj=9, got {}", obj_val);
    // Verify knapsack constraint is satisfied
