...
/// Apply a delta batch, asserting it fully applies and leaves the cursor Ready.
///
/// Mirrors the `BackendSession::synchronize(DeltaBatch)` contract for the
/// reference backend: on `Applied` the cursor advances to `batch.to` and
/// health stays `Ready`; any other outcome is a test failure.
fn apply_ok(backend: &mut ReferenceBackend, cursor: &mut AdapterCursor, batch: &DeltaBatch) {
    match backend
        .apply_batch(batch, cursor)
        .expect("apply_batch should not fail")
    {
        ApplyOutcome::Applied { new_revision } => {
            assert_eq!(new_revision, batch.to, "Applied revision must be batch.to");
            assert!(
                cursor.is_ready(),
                "cursor must be Ready after successful apply"
            );
        }
        other => panic!(
            "expected ApplyOutcome::Applied for r{}→r{}, got {other:?}",
            batch.from, batch.to
        ),
    }
}

/// Rebuild a backend from a snapshot, asserting the cursor advances to the
...
fn assert_commuting_square(snap_r0: &ModelSnapshot, snap_r1: &ModelSnapshot, batch: &DeltaBatch) {
    ...
    rebuild_ok(&mut backend_a, &mut cursor_a, snap_r1);
    let view_a = backend_a.normalized_view();

    let mut backend_b = fresh();
    let mut cursor_b = AdapterCursor::new();
    rebuild_ok(&mut backend_b, &mut cursor_b, snap_r0);
    apply_ok(&mut backend_b, &mut cursor_b, batch);
...
fn assert_commuting_square_multi(
    ...
    let mut backend_a = fresh();
    let mut cursor_a = AdapterCursor::new();
    rebuild_ok(&mut backend_a, &mut cursor_a, snap_end);
    let view_a = backend_a.normalized_view();
...
fn dx_random_mutation_sequences() {
    ...
    let mut tracker_cursor = AdapterCursor::new();
    rebuild_ok(&mut tracker_backend, &mut tracker_cursor, &snap_r0);

    for (j, batch) in batches.iter().enumerate() {
        // Apply the batch on the tracker backend
        apply_ok(&mut tracker_backend, &mut tracker_cursor, batch);
        ...
        let mut snap_cursor = AdapterCursor::new();
        rebuild_ok(&mut snap_backend, &mut snap_cursor, &snap_ref);
        let snap_view = snap_backend.normalized_view();

        let mut seq_backend = fresh();
        let mut seq_cursor = AdapterCursor::new();
        rebuild_ok(&mut seq_backend, &mut seq_cursor, &snap_r0);
        for k in 0..=j {
            apply_ok(&mut seq_backend, &mut seq_cursor, &batches[k]);
        }
...
fn dx_multi_adapter_cursor_independence() {
    ...
    rebuild_ok(&mut backend_a, &mut cursor_a, &ModelSnapshot::empty(r0));
    let batches_a = coordinator.batches_for_cursor(&cursor_a).unwrap();
    for batch in &batches_a {
        apply_ok(&mut backend_a, &mut cursor_a, batch);
    }
    ...
    let mut backend_b = fresh();
    let mut cursor_b = AdapterCursor::new();
    rebuild_ok(&mut backend_b, &mut cursor_b, &ModelSnapshot::empty(r0));
    ...
    let mut ref_cursor = AdapterCursor::new();
    rebuild_ok(&mut ref_backend, &mut ref_cursor, &snap_r1);
    assert_eq!(
...
fn dx_rebuild_determinism() {
    ...
    let mut backend_a = fresh();
    let mut cursor_a = AdapterCursor::new();
    rebuild_ok(&mut backend_a, &mut cursor_a, &snapshot);

    let mut backend_b = fresh();
    let mut cursor_b = AdapterCursor::new();
    rebuild_ok(&mut backend_b, &mut cursor_b, &snapshot);
...
fn dx_semicontinuous_partial_apply() {
    ...
    let mut cursor = AdapterCursor::new();
    rebuild_ok(&mut backend, &mut cursor, &ModelSnapshot::empty(r0));
    apply_ok(&mut backend, &mut cursor, &batch1);
    assert_eq!(cursor.applied_revision, r1);
    ...
    rebuild_ok(&mut backend, &mut cursor, &snap_r2);
    ...
    rebuild_ok(&mut direct_backend, &mut direct_cursor, &snap_r2b);
