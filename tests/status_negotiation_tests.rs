...
fn config_adjustment_carries_all_fields() {
    let adj = ConfigAdjustment {
        ...
        requested: "DualSimplex".into(),
        applied: "DualSimplex".into(),
        reason: "solver does not support DualSimplex".into(),
    };
    assert_eq!(adj.key, "lp_algorithm");
    assert_eq!(adj.requested, "DualSimplex");
    assert_eq!(adj.applied, "DualSimplex");
    assert_eq!(adj.reason, "solver does not support DualSimplex");
}
