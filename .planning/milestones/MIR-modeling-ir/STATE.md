...
| MIR-06 | in progress (IR-27, IR-28): M6-0B + M6-1A + M6-1B/C + M6-2A complete (Python arrays wrap shared handles; `GeneralLinArray` fallback in `roml::modeling`); M6-2B next | `MIR-06-PLAN.md`; `evidence/MIR-06-REPORT.md`; `roml-python/src/arrays.rs`; `src/modeling/general.rs`; `Model::{normalized_*_fingerprint,add_*_array_block,set_parameter_array}`; `python/tests/{test_fingerprint,test_ir27_gate}.py`; `tests/mir06_*.rs` |
| MIR-07 | not started | — |
| MIR-08 | not started | — |
