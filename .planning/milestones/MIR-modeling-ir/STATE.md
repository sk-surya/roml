...
| MIR-06 | in progress (IR-27, IR-28): M6-0B + M6-1A + M6-1B/C complete (Python arrays wrap shared handles; gathered-ID layer deleted); M6-2 next after review checkpoint | `MIR-06-PLAN.md`; `evidence/MIR-06-REPORT.md`; `roml-python/src/arrays.rs`; `Model::{normalized_*_fingerprint,add_*_array_block,set_parameter_array}`; `modeling::{View,VarArray,ParamArray}::{subsample,squeeze}`; `python/tests/{test_fingerprint,test_ir27_gate}.py`; `tests/mir06_*.rs` |
| MIR-07 | not started | — |
| MIR-08 | not started | — |
