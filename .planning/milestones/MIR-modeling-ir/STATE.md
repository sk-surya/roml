...
| MIR-04 | in progress (IR-24, IR-25): M4-1 handles + M4-2 expression algebra + M4-3 label boundary + M4-4 normalized ordinal-IR fingerprint done | `MIR-04-PLAN.md`; `src/modeling/{array,coeff,builder,labeled}.rs`; `src/snapshot.rs`; `tests/mir04_{arrays,expr,labels,fingerprint}.rs` |
| MIR-05 | not started | — |
| MIR-06 | not started | — |
| MIR-07 | not started | — |
| MIR-08 | not started | — |
