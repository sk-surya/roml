...
Post-P25 `cargo public-api -p roml` capture: `docs/release/evidence/M3_P25_public_api_roml_final.txt` (12016 lines; baseline 10737). Diff (distinct `pub`-item lines): **+410 added / −4 removed**. The four removed: `Change::affects_solver` (IN-01 dead code), the pre-fix `normalize_result` signature (CR-02 identity parameters), and two `Model::clone`/`Model::default` lines from derived→manual impl rendering (methods remain public with identical signatures, confirmed by `tests/public_api_compile.rs`; SM-15.1 M2 surface preserved). Re-verification round (blocking review): **zero** fixture symbols (`FixturePayload`/`ConstructData`/`ConstructKind`/`ConstructEntry`/`add_construct_fixture`) remain in the public API (0 matches in the final capture).
...
### Second review round — blocking independent review (PR #27)

Contract-level findings from the independent review; all six verified against the code and fixed with TDD on the branch:

1. **F1 — symbolic parameter expressions lost in the semantic IR.** `FunctionEntry` was rebuilt from evaluated `f64` only; the coefficient index and snapshot `CellEntry` retain `ValueExpr` + dependencies. First fixed in `be67053` with a parallel symbolic view (`terms`/`dependencies` fields); the second review rejected that shape (multiple authorities; violates design §6, where the symbolic expression belongs inside `ScalarFunction` and `FunctionConstraint` is only `{ function, set }`). Re-fixed in `791708f`: canonical functions are reconstructed symbolically — `ScalarFunction::Linear(LinExpr)` carries `TermCoeff::Expr(ValueExpr)` coefficients sourced from the coefficient index's `value_expr` — in `constraint_function` (via `constraint_symbolic_expression`), snapshot, and delta reconstruction; the parallel fields were removed; `parameter_dependencies()` is derived (on `LinExpr`/`ScalarFunction`/`FunctionConstraint`), never stored; `constraint_expression` remains the evaluated convenience API. Test `semantic_ir_preserves_symbolic_parameter_terms`: `p * x` yields `TermCoeff::Expr` referencing `p` and derived `parameter_dependencies() == [p]` across `constraint_function`, snapshot, delta, and after a parameter update; the `take_snapshot` function-equality invariant holds with symbolic functions.
2. **F2 — `DeltaBatch::functions` incomplete semantic delta.** Fixed in `c0fc186`: an `AddConstraint` with a later same-batch `RemoveConstraint` contributes no entry; the narrowed contract is documented — `functions` is the view of constraints added with final folded bounds minus constraints removed; updates to pre-existing functions ride the ops; full before/after semantic entries deferred until recipe-level incremental equivalence is proven (design §8). Contract tests added in `1f78e39`: add-then-remove yields no stale entry; `delta_functions_contract_coefficient_update_rides_ops_not_functions_view` (existing-row parameter update → no entry, ops carry `SetCell`); `delta_functions_contract_existing_row_removal_rides_ops_not_functions_view` (existing-row removal → no entry, ops carry `RemoveConstraint`).
3. **F3 — private lifecycle fixture leaked into the public API.** `FixturePayload`, `ConstructKind::Fixture`, `ConstructData`, `add_construct_fixture` were public. Fixed in `f384ff9`/`1db880d`: the construct module and fixture scaffolding are crate-private; only `Construct` and `FormulationPreference` remain public exports; construct-lifecycle tests moved in-crate (`#[cfg(test)]` in `src/model/mod.rs`); `ModelSnapshot.constructs` stays `pub #[doc(hidden)]` so external crates can build snapshot literals. Final public-API capture contains **zero** fixture symbols.
4. **F4 — `FormulationPreference` dropped by snapshots/deltas.** Fixed in `94d3165`: `preference` threaded through `ConstructEntry` (new field, single authority), `Change::ConstructAdded`, `ModelOp::AddConstruct`, and the snapshot/delta reconstruction; `ConstructData.preference` removed. Test: `NativeRequired` round-trips through entry, snapshot, delta, and rebuild-from-snapshot.
5. **F5 — two construct metadata authorities.** `ConstructData.metadata` was dead. Fixed in `23ef795`: field removed; single authority = model-level `EntityRef::Construct`-keyed metadata map.
6. **F6 — traceability overstates closure.** TRACEABILITY corrected to clause-level status: SM-01.1–01.3/01.5/01.6 closed in P25 (SM-01.4 partial — objective policies P31); SM-02.1/02.3/02.7 closed (02.2 foundations; 02.4/02.5 → P26; 02.6 → P29).

Contract amendments recorded in DECISIONS.md (A29–A31); re-verification: `cargo test -p roml --all-targets` **598 pass**, clippy `-D warnings` clean, `roml-highs` 100 pass, public API fixture-free.
