...
1. **F1 — symbolic parameter expressions lost in the semantic IR.** `FunctionEntry` was rebuilt from evaluated `f64` only; the coefficient index and snapshot `CellEntry` retain `ValueExpr` + dependencies. Fixed in `be67053`: `FunctionEntry`/`FunctionConstraint` gained the public symbolic view `terms: Vec<(VarId, ValueExpr)>` + `dependencies: Vec<ParamId>`, populated from the coefficient index in `constraint_function`, snapshot, and delta reconstruction (the `ModelOp::SetCell` op already carried `value_expr`). Test: `p * x` keeps `ValueExpr` referencing `p` (not just the number) and `dependencies == [p]` across `constraint_function`, snapshot, delta, and after a parameter update.
2. **F2 — `DeltaBatch::functions` incomplete semantic delta.** Fixed in `c0fc186`: an `AddConstraint` with a later same-batch `RemoveConstraint` contributes no entry; the narrowed contract is documented — `functions` is the view of constraints added with final folded bounds minus constraints removed; updates to pre-existing functions ride the ops; full before/after semantic entries deferred until recipe-level incremental equivalence is proven (design §8). Tests: add-then-remove yields no stale entry; contract assertions.
