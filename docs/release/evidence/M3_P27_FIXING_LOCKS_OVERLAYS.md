...
### Second review round — blocking independent review (PR #29, trust boundary)

Four findings verified and fixed with TDD:
1. **F1 overlay bounds overwrite instead of intersect** (`e6a24d0`) — compile_overlay aggregates per-variable restrictions (temp fixings + all lock bounds) before producing ops, intersects against `effective_bounds()` (declared ∩ persistent fixing ∩ activity), emits at most one temporary-bound op per variable, rejects empty intersections (`OverlayError::InfeasibleOverlay`) before any backend mutation; validation and Within clipping operate on effective bounds. Regressions: conflicting temp+exact lock, disjoint bands, persistent-fix conflict, inactive-variable [0,0] base, across compiler + Reference + HiGHS/facade.
2. **F2 complete CompiledOverlay preflight** (`8dd98df`) — reusable `CompiledOverlay::validate` against the live registry: `compilation_id != base_compilation`, order-simulated ops (row-id collisions, missing refs, rollback-only ops in apply stream, dangling policies), origins for every added row; both backends preflight before any native mutation; malformed-overlay tests prove native/compiled state and exact identity unchanged.
3. **F3 RequiresRebuild outcomes mark the session** (`3b1c4e4`) — full `(overlay_id, base_compilation, applied_compilation)` receipt tuple validated; HiGHS marks RequiresRebuild on every uncertainty path; the façade defensively `force_rebuild_on_next_sync()` on every such outcome; forged-receipt regressions prove the next plain solve rebuilds with no overlay leak.
4. **F4 non-finite cutoffs rejected** (`483122f`) — `OverlayError::InvalidCutoff`; non-finite `ObjectiveCutoff.limit` rejected during compilation before any operation; NaN/±inf regressions on compiler, façade, and HiGHS paths.

Re-verification: `cargo test -p roml --all-targets` **759 pass**, `roml-highs` **123 pass**, clippy `-D warnings` clean. M2 guarded surface unchanged.
