...
grep 'license' Cargo.toml  # at commit c4db3e0
```
...
|------|------------|--------|-------|
| `LICENSE-MIT` | `c4db3e0` | Present | MIT License, Copyright (c) 2026 Surya Krishnan |
| `LICENSE-APACHE` | `c4db3e0` | Present | Apache License 2.0, January 2004 |
| `Cargo.toml` (workspace.package license) | `c4db3e0` | `"MIT OR Apache-2.0"` | Field: `license = "MIT OR Apache-2.0"` in `[workspace.package]`. Declared at line 11 of `Cargo.toml` at commit `c4db3e0`. All four workspace members (`roml`, `roml-highs`, `roml-mosek`, `roml-xpress`) inherit via `license.workspace = true`. |
...
```bash
# Attempt 1: crates.io API endpoint (requires authentication per data access policy)
...
# Full API response (with auth required)
